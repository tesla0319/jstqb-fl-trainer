...
| 機能 | 説明 |
|---|---|
| **40問トレーニングモード** | 通常モードで40問1セット（カテゴリ比率ベースで自動配分） |
| **ランダム出題** | 全カテゴリからランダムに1問出題 |
| **カテゴリ指定出題** | 指定カテゴリから問題を出題 |
| **重複出題防止** | セッション内で回答済みの問題は再出題しない（`exclude_ids` により制御） |
| **選択肢シャッフル** | 問題ごとに選択肢表示順をランダム化（正答位置の偏りを防止） |
...
| `limit` | `40` | 取得件数上限（1以上）。`random=true` のときは無視 |
| `exclude_ids` | なし（複数可） | 除外する問題 ID。セッション内重複防止に使用 |
...
{
  "categories": [
    { "id": 1, "name": "テストの基礎", "count": 18 },
    { "id": 2, "name": "テスト活動とプロセス", "count": 17 }
  ]
}
```

`count` は登録済み問題数（カテゴリ比率配分の自動計算に使用）。
